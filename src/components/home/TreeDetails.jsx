...
import { restartTrackingListTree, setCart, setPathName, setQuantity } from "../../actions/action";

const TreeDetails = () => {
    const { oneTreeInfo } = useSelector(state => state.mainReducer)
    const { listTrackingListTree, currentUser } = useSelector(state => state.accountReducer)
    const { quantity } = useSelector(state => state.subAddReducer)
    ...
    useEffect(() => {
        dispatch(setPathName(oneTreeInfo.treeName))
        RootService.ImageService.getTreeImage(oneTreeInfo.treeId).then(
            (response) => {
    ...
    useEffect(() => {
        ...
        listTrackingListTree.map(check => {
            if (check.treeId === oneTreeInfo.treeId) {
                setChecked(true)
                return true
            } else return false;
        })
    ...
    const hangleClickSub = () => {
        if (quantity > 0)
            dispatch(setQuantity(parseInt(quantity) - 1))
    }

    const hangleClickAdd = () => {
        if (quantity < oneTreeInfo.stock)
            dispatch(setQuantity(parseInt(quantity) + 1))
    }
    ...
    const warning = [
        <Notification
            ...
            pushMessage={'Không thể thêm ' + oneTreeInfo.treeName + ' vào giỏ hàng.'}
        />,
        <Notification
            pushType='warning'
        ...
        <Notification
            ...
            pushMessage={'Đã thêm ' + quantity + ' ' + oneTreeInfo.treeName + ' vào giỏ.'}
        />
    ]
    ...
    const handleAddToCart = () => {
        if (!currentUser) {
            ...
            RootService.CartService.addTreeToCart(oneTreeInfo.treeId, currentUser.id, quantity).then(
                (response) => {
    ...
    const handleClickFollow = () => {
        ...
        dispatch(restartTrackingListTree(currentUser.id, oneTreeInfo.treeId))
    }
                    ...
                    <div className="row">
                        <aside className="col-lg-6">
                            <article className="gallery-wrap">
                                <div className="img-big-wrap img-thumbnail">
                                    <a data-fslightbox="mygalley" data-type="image" href={image}>
                                        <img height="560" src={image} alt="" />
                                    </a>
                                </div>
                                <div className="thumbs-wrap">
                                    <a data-fslightbox="mygalley" data-type="image" href={image} className="item-thumb">
                        ...
                        <main className="col-lg-6">
                            <article className="ps-lg-3">
                                <h4 className="title text-dark">{oneTreeInfo.treeName}</h4>
                                ...
                                <div className="mb-3">
                                    <var className="price h5">{oneTreeInfo.price.toLocaleString('vi-VN')}<sup>₫</sup></var>
                                    <span className="text-muted">/1 cây</span>
                                </div>

                                <p>{oneTreeInfo.description}.</p>

                                <dl className="row">
                                    <dt className="col-3">Màu cây</dt>
                                    <dd className="col-9">{oneTreeInfo.color}</dd>

                                    <dt className="col-3">Kích thước</dt>
                                    <dd className="col-9">{oneTreeInfo.size}</dd>

                                    <dt className="col-3">Thể loại</dt>
                                    <dd className="col-9">
                                        {
                                            oneTreeInfo.categories.map(category => {
                                                return (
                                ...
                                <div className="row mb-4">
                                    <span className="label-rating text-muted">
                                        ...
                                        {' '}còn {oneTreeInfo.stock} sản phẩm
                                    </span>
                            ...
                            <div className="card">
                                ...
                                <div className="tab-content">
                                    ...
                                    <article id="tab_warranty" className="tab-pane card-body">
                                        <h4>{oneTreeInfo.supplier.supplierName}</h4>
                                        <br />
                        ...
                        <aside className="col-lg-4">
                            ...
                            <div className="card">
                                <div className="card-body">
                                    ...
                                    <article className="itemside mb-3">
                                        <a href="#" className="aside">
                                            <img src={image} width="96" height="96" className="img-md img-thumbnail" />
                                        </a>
                                        <div className="info">
